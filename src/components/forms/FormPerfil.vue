...
const checkData = (): {status: boolean, message?: string} => {
  ...
  if (dataPerfil.name.trim().length < 2) {
    return { status: false, message: 'Nome de usuário deve ter mais de 2 caracteres' };
  }
  ...
  if (dataPerfil.passwordActual.trim().length > 0) {
    ...
    if (dataPerfil.passwordActual.trim().length < 7) {
      return { status: false, message: 'A senha deve conter mais de 7 caracteres' };
    }
    if (dataPerfil.passwordNew.trim() === '') {
      return { status: false, message: 'Deve ser informado a nova senha do usuário' };
    }
    if (dataPerfil.passwordActual.trim().length < 7) {
      return { status: false, message: 'A nova senha deve conter mais de 7 caracteres' };
    }
...
watch(() => dataPerfil.passwordActual, (password) => {
